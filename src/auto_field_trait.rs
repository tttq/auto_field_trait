...
/// 自动字段处理核心 Trait (内部使用，开发者无需手动调用)
pub trait AutoFieldHandler {
    /// 插入时的字段初始化 (由 ActiveModelBehavior::before_insert 自动调用)
    fn auto_fill_create_fields(&mut self, context: &AutoFieldContext);

    /// 更新时的字段初始化 (由 ActiveModelBehavior::before_update 自动调用)
    fn auto_fill_update_fields(&mut self, context: &AutoFieldContext);

    /// 软删除时的字段初始化 (由软删除方法内部调用)
    fn auto_fill_soft_delete_fields(&mut self, context: &AutoFieldContext);
}
...
/// 查询扩展 Trait (用于查询数据库记录，不是获取当前操作人)
pub trait QueryExtensions: EntityTrait {
    ...
    /// 按租户名称查询 (查找属于某个租户的记录)
    fn find_by_tenant_name(tenant_name: &str) -> Select<Self>;
